...
def fill_books(dataframe, needed_book_count):
    ...
    while dataframe.tail(1).iloc[0]["누적권수"] <= needed_book_count:
        # URL 목록 정의
        search_urls = [
            f'http://www.aladin.co.kr/ttb/api/ItemList.aspx?ttbkey={common.aladin_key}&QueryType=ItemNewAll&MaxResults=200&Start={count}&SearchTarget=Book&output=js&Version=20131101',
            f'http://www.aladin.co.kr/ttb/api/ItemList.aspx?ttbkey={common.aladin_key}&QueryType=Bestseller&MaxResults=200&Start={count}&SearchTarget=Book&output=js&Version=20131101',
            f'http://www.aladin.co.kr/ttb/api/ItemList.aspx?ttbkey={common.aladin_key}&QueryType=BlogBest&MaxResults=200&Start={count}&SearchTarget=Book&output=js&Version=20131101',
            f'http://www.aladin.co.kr/ttb/api/ItemList.aspx?ttbkey={common.aladin_key}&QueryType=ItemNewSpecial&MaxResults=200&Start={count}&SearchTarget=Book&output=js&Version=20131101'
        ]
...
def main(needed_input):        
    ...
    result_df.to_csv(f'{common.save_file_path}/combined_book_recommendations.csv', index=False, encoding='utf-8')
    delete_all_files()
